...
// @version      0.3.4
// @description  Пару инструментов для зеро блока в Тильде
// @author       Тильдошная
...
(function() {
	...
	setInterval(function(){
		if ($(".t396__iframe").length > 0 && $(".t396__iframe").hasClass("modified") == false) {
			zeroButtonOpen();
		}
	}, 500);
	...
	function zeroButtonOpen(event) {
		console.log("zeroButtonOpen");
		$(".t396__iframe").addClass("modified");
		// var iframe = document.getElementsByTagName('iframe')[0];
		var iframe = $(".t396__iframe.modified").get(0);
		...
		iframe.onload = function() {
			...
			zeroInit(iframeDoc2);
			extensionInitIndicator(iframeDoc2);
		};
	}

	function extensionInitIndicator(iframeDoc2) {
		$(iframeDoc2).find("body").prepend("<div class='init_complete' title='Расширение запущено' style='border-radius: 50%; height: 20px; width: 20px; background: #11c311; position: fixed; top: 20px; left: 72px; z-index: 999999999; box-shadow: 0 0 5px #11c311;'> </div>");
		setTimeout(function(){
			$(iframeDoc2).find(".init_complete").fadeOut("fast");
		}, 2000);
	}
	...
	// Запуск зеро блока
	function zeroInit(iframeDoc2) {
		setTimeout(function() {
			if ($(iframeDoc2).find(".tn-canvas-max-bottom").length == 0) {
				init();
				return;
			} else {
				console.log("init complete");
				// Клик на элемент
				$(iframeDoc2).find(".tn-elem").on("click", function(){
						multipleItems(iframeDoc2);
						singleItem(iframeDoc2);
						setTimeout(tooltips(iframeDoc2), 100);
				});
				// Смена адаптива
				$(iframeDoc2).find(".tn-res-item").on("click", function(){
						multipleItems(iframeDoc2);
						singleItem(iframeDoc2);
						setTimeout(tooltips(iframeDoc2), 100);
				});
				// Лассо
				$(iframeDoc2).find(".tn-floor").on("mouseup", function(){
						multipleItems(iframeDoc2);
						singleItem(iframeDoc2);
						setTimeout(tooltips(iframeDoc2), 100);
				});
				$(iframeDoc2).on("keydown", function(e) {
					// cntrl + a
					if ((e.metaKey || e.ctrlKey) && 65 == e.keyCode) {
						setTimeout(function(){
							multipleItems(iframeDoc2);
							singleItem(iframeDoc2);
							setTimeout(tooltips(iframeDoc2), 100);
					...
					// cntrl + 1-5
					if ((e.metaKey || e.ctrlKey) && 48 < e.keyCode < 54 )  {
						setTimeout(function(){
							multipleItems(iframeDoc2);
							singleItem(iframeDoc2);
							setTimeout(tooltips(iframeDoc2), 100);
	...
	///////////////////////////////////////////////
	function tooltips(iframeDoc2) {
	  $('.t396__iframe.modified').get(0).contentWindow.tn_tooltip_update();
	}

	function render() {
		$('.t396__iframe.modified').get(0).contentWindow.elem__selected__offsetMove(0, -100);
		$('.t396__iframe.modified').get(0).contentWindow.elem__selected__offsetMove(0, 100);
	}
...
// hexToRgb(hex)
	...
	///////////////////////////////////////////////
	///////// функции внутри зеро блока////////////
	///////////////////////////////////////////////
	function singleItem(iframeDoc2) {
	...
	function multipleItems(iframeDoc2) {
		if ($(iframeDoc2).find(".tn-elem__selected:not(.tn-elem__invisible)").length > 1) {
			...
			if ($(iframeDoc2).find(".sui-panel__section-anim").length == 0) {
				...
				$(iframeDoc2).find(".sui-btn-anim-test-all").click(function(){
					$('.t396__iframe.modified').get(0).contentWindow.tn_anim__playAll();
				})
			}
			// /Стандартная анимация
			...
			setTimeout(function(){
				$(iframeDoc2).find(".tn-right-box input").each(function(){
					this.onfocus = function(){
						$('.t396__iframe.modified').get(0).contentWindow.tn_flag_settings_ui_focus = true;
					}
					this.onblur = function() {
						$('.t396__iframe.modified').get(0).contentWindow.tn_flag_settings_ui_focus = false;
					}
				})
